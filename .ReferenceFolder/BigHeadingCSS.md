...
.welcome-branding-panel .welcome-logo {
  --welcome-logo-size: clamp(48px, 14vw, 72px);
  display: block;
...
@media (max-width: 520px) {
  ...
  .welcome-branding-panel .welcome-logo {
    --welcome-logo-size: clamp(40px, 12vw, 56px);
  }

  .welcome-branding-panel h2 {
    font-size: clamp(1.2rem, 29vw, 2.2rem);
  }
...
@media (min-width: 869px) {
  ...
  body[data-page="rosters"] #header-container .app-header {
    ...
    --roster-desktop-field-width: 313px;
    --roster-desktop-max: min(calc(500vw - 0rem), 5600px);
    display: grid;
...
@media (min-width: 869px) {
  ...
  .compact-switcher button {
    padding: 0.6rem 1.4rem 0.2rem !important;
    min-width: 38px;
  }
...
.team-record-champ {
  color: #D0B472;
  margin-left: 0.5rem;
  filter: drop-shadow(0 0 4px rgba(208, 180, 114, 0.6));
  font-size: 0.85rem;
}
...
.game-logs-loading-container {
  position: absolute;
  top: 32px;
  /* Below header - adjust based on actual header height */
  left: 0;
  right: 0;
  bottom: 0;
  display: flex;
...
/* === Loading Overlay + Orbit Ring (uses existing theme colors) === */
#loading.hidden {
  display: none;
}
...
#loading .loading-logo {
  ...
  /* reuse the welcome mask look (soft vignette) */
  -webkit-mask-image: radial-gradient(circle at 52% 51%, rgba(0, 0, 0, 0.85) 42%, rgba(0, 0, 0, 0) 99%);
  mask-image: radial-gradient(circle at 52% 51%, rgba(0, 0, 0, 0.85) 42%, rgba(0, 0, 0, 0) 99%);
  -webkit-mask-repeat: no-repeat;
  mask-repeat: no-repeat;
  -webkit-mask-size: 100% 100%;
  mask-size: 100% 100%;
...
#loading .loading-ring .ch {
  position: absolute;
  top: 50%;
  left: 50%;
  transform:
    rotate(calc(var(--angle) + var(--offset) + (360deg / var(--n)) * var(--i))) translate3d(0, calc(-1 * var(--r)), 0);
  transform-origin: center;
  font-weight: 600;
  font-size: 16px;
  letter-spacing: 0.08em;
...
#loading .loading-text .loading-message {
  display: inline-block;
  font-size: 0.95rem;
  color: var(--color-text-mid-test1, #BDC1D8);
  text-shadow: 0 0 10px rgba(128, 138, 189, 0.12);
}

/* === End Loading Overlay === */
/* Optical center fix: nudge logo down slightly */
/* === Loading logo robust centering (prevents rare off-center shift on mobile) === */
#loading .loading-logo-wrap {
  position: absolute;
  inset: auto;
  top: 50%;
  ...
  transform: translate3d(-50%, -50%, 0);
  display: block;
  /* override grid centering to avoid rounding issues */
}

#loading .loading-logo {
  display: block;
  transform: translateY(var(--loading-logo-nudge, 3px));
  will-change: transform;
  -webkit-transform: translateY(var(--loading-logo-nudge, 3px));
  -webkit-font-smoothing: antialiased;
  backface-visibility: hidden;
}

/* === End robust centering === */
/* === Orbit ring stability tweaks (prevent perceived center shift) === */
#loading .loading-scene {
  width: 320px;
  /* integer size helps avoid half-pixel rounding */
  height: 320px;
  transform: translateZ(0);
...
/* Compute the angle as a variable to avoid repeated calc jitter */
#loading .loading-ring {
  --n: 28;
  --r: 120px;
  --offset: -120deg;
...
/* === End orbit ring stability tweaks === */
/* === Loader (mobile-safe): radial ring + fixed center + logo mask/glow === */
/* Scene box (integer size avoids sub-pixel wobble) */
#loading .loading-scene {
  position: relative;
  width: 320px;
  height: 320px;
...
/* Ring: center is static; rotation driven by --angle (set by JS) */
#loading .loading-ring {
  --n: 28;
  /* character count */
  --r: 120px;
  ...
  left: 50%;
  transform: translate3d(-50%, -50%, 0);
  /* center does NOT animate */
  transform-origin: 50% 50%;
  animation: none !important;
  /* ensure no keyframe rotation */
  will-change: transform;
}

/* Radial letters: bottoms point toward center */
#loading .loading-ring .ch {
  position: absolute;
  top: 50%;
  left: 50%;
...
/* Logo wrapper: transform-free centering (robust on iOS) */
#loading .loading-logo-wrap {
  position: absolute;
  inset: 0;
  /* top/right/bottom/left: 0 */
  width: 132px;
  /* fixed box = stable masking */
  height: 132px;
  margin: auto;
  /* centers both axes (no %) */
  display: block !important;
  /* defeat any grid/other display */
  transform: translate3d(4px, 7px, 0) !important;
  /* <- pixel nudge: right 4px, down 7px */
  pointer-events: none;
  contain: layout paint style;
}

/* Move your mask + glow onto the IMG so it’s stable on mobile */
#loading .loading-logo {
  display: block;
  width: 100%;
  height: 100%;
  -webkit-mask-image: radial-gradient(circle at 52% 51%, rgba(0, 0, 0, 0.81) 40%, rgba(0, 0, 0, 0) 99%);
  mask-image: radial-gradient(circle at 52% 51%, rgba(0, 0, 0, 0.81) 30%, rgba(0, 0, 0, 0.01) 99%);
  -webkit-mask-repeat: no-repeat;
  mask-repeat: no-repeat;
  -webkit-mask-size: 100% 100%;
  mask-size: 100% 100%;
  filter: drop-shadow(2px 2px 4px #5900ffed) drop-shadow(0px 0px 11px #7300ff58);
  transform: none !important;
...
#loading .loading-logo-wrap::after {
  content: "";
  position: absolute;
  inset: -4px;
  border-radius: 14px;
