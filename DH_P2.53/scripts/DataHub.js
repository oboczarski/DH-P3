...
// TM and the default CFB header share this Material-style path treatment, while
// the Rookies CFB table header gets its own requested shield/check glyph below.
...
function getActiveColumnIconMarkup(columnName) {
  ...
  if (isDataHubRookiesView()) {
    ...
    if (columnName === "CFB") {
      return ROOKIE_CFB_HEADER_ICON_MARKUP;
    }
  }
...
function createHeaderCell(column, columnIconColor) {
  ...
  if (iconMarkup) {
    ...
    if (column.name === "TIER" && isDataHubRookiesView()) {
      ...
      // the requested Material-style tier-node glyph uses a non-24x24 viewBox
    ...
    if (column.name === "CFB" || column.name === "TM") {
      ...
      // both columns use the same Material viewBox/fill class; rookie CFB can
      // still provide its own path through getActiveColumnIconMarkup above.
      svg.setAttribute("viewBox", DATAHUB_MATERIAL_SYMBOL_VIEW_BOX);
