...
const DEFAULT_DATAHUB_THEME = "halo";
const DATAHUB_THEME_STORAGE_KEY = "dh_datahub_theme";
const DATAHUB_THEME_VARIANTS = Object.freeze(["halo", "ember", "contour"]);
const DATAHUB_THEME_SET = new Set(DATAHUB_THEME_VARIANTS);
...
const themeButtons = Array.from(document.querySelectorAll("[data-theme-variant]"));
...
// DataHub theme controller: only shell visuals change here. Theme swaps should
// not touch table state, data fetches, or any of the page's non-style behavior.
...
function attachThemeSwitcherListeners() {
  themeButtons.forEach((button) => {
    button.addEventListener("click", () => {
      const requestedTheme = button.dataset.themeVariant;
      if (!isValidDataHubTheme(requestedTheme) || requestedTheme === state.themeVariant) {
        return;
      }

      setDataHubTheme(requestedTheme);
    });
  });
}
...
function persistThemeVariant(themeVariant) {
  try {
    ...
    // Storage can fail in privacy-restricted contexts; keep the theme active
    // in memory and continue without blocking the rest of the page.
  }
}
