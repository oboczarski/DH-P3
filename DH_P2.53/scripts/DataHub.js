...
// the rookie-tab tables use the requested filled Material-style branch path
// and get a separate viewBox/fill treatment from the default stroke icons.
const ROOKIE_TIER_HEADER_ICON_MARKUP = '<path d="M114.5-160v-120q0-19.15 13.17-32.33Q140.85-325.5 160-325.5h119.52l156.89-156.65v-121.07q-36.95-13.95-59.43-45.65-22.48-31.7-22.48-71.13 0-52.39 36.55-88.95Q427.61-845.5 480-845.5t88.95 36.55Q605.5-772.39 605.5-720q0 39.43-22.48 71.13-22.48 31.7-59.43 45.65v121.07L680.48-325.5H800q19.15 0 32.33 13.17Q845.5-299.15 845.5-280v120q0 19.15-13.17 32.33Q819.15-114.5 800-114.5H680q-19.15 0-32.33-13.17Q634.5-140.85 634.5-160v-83.91L480-398.41l-154.5 154.5V-160q0 19.15-13.17 32.33Q299.15-114.5 280-114.5H160q-19.15 0-32.33-13.17Q114.5-140.85 114.5-160Z" />';
...
// TM and CFB share the same requested Material-style path treatment, while
// MTF uses its own filled icon and YCO/A reuses the former MTF Zap glyph so
// every affected table keeps a local inline SVG.
...
const COLUMN_ICONS = {
  ...
  TM:        CFB_HEADER_ICON_MARKUP, // Filled team icon matches current CFB header icon
...
function createHeaderCell(column, columnIconColor) {
  ...
  if (iconMarkup) {
    ...
    if (column.name === "CFB" || column.name === "TM") {
      ...
      // both columns now use the same filled Material glyph and shared class
      // so the table renders them through the exact same viewBox/fill path.
      svg.setAttribute("viewBox", DATAHUB_MATERIAL_SYMBOL_VIEW_BOX);
