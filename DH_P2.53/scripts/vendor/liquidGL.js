...
(() => {
  ...
  class liquidGLRenderer {
    constructor(snapshotSelector, snapshotResolution = 1.0, rendererOptions = {}) {
      ...
      this._snapshotAllowsFixed = !!rendererOptions.allowFixedSnapshot;
      this._scrollWatchRaf = null;
      this._scrollTimeout = null;
      this._onResize = null;
      this._snapshotResizeObserver = null;
      this._dynamicStyleHost = null;
      ...
      const scrollCheck = () => {
        ...
        this._scrollWatchRaf = requestAnimationFrame(scrollCheck);
      };
      this._scrollWatchRaf = requestAnimationFrame(scrollCheck);
      ...
      if ("ResizeObserver" in window) {
        this._snapshotResizeObserver = new ResizeObserver(onResize);
        this._snapshotResizeObserver.observe(this.snapshotTarget);
      }
      ...
      this._dynamicStyleHost = styleEl;
        ...
        try {
          ...
          const ignoreElementsFunc = (element) => {
            ...
            if (style.position === "fixed") {
              if (this._snapshotAllowsFixed && element === this.snapshotTarget) {
                return false;
              }
              return true;
            }
    ...
    destroy() {
      ...
      if (this._scrollWatchRaf) {
        cancelAnimationFrame(this._scrollWatchRaf);
        this._scrollWatchRaf = null;
      }
      ...
      if (this._snapshotResizeObserver) {
      ...
      this.lenses.forEach((lens) => {
        if (lens && typeof lens.destroy === "function") {
          lens.destroy();
        }
      });
      this.lenses = [];
      ...
      if (this._dynamicStyleHost && this._dynamicStyleHost.parentNode) {
        this._dynamicStyleHost.parentNode.removeChild(this._dynamicStyleHost);
      }
  ...
  class liquidGLLens {
    ...
    destroy() {
      ...
      this.el.style.background = this._originalInlineStyles.background;
      this.el.style.backgroundColor = this._originalInlineStyles.backgroundColor;
      this.el.style.backgroundImage = this._originalInlineStyles.backgroundImage;
      this.el.style.backdropFilter = this._originalInlineStyles.backdropFilter;
      this.el.style.boxShadow = this._originalInlineStyles.boxShadow;
      this.el.style.opacity = this._originalInlineStyles.opacity;
      this.el.style.pointerEvents = this._originalInlineStyles.pointerEvents;
      this.el.style.position = this._originalInlineStyles.position;
      this.el.style.transition = this._originalInlineStyles.transition;
      this.el.style.transform = this._originalInlineStyles.transform;
      this.el.style.transformOrigin = this._originalInlineStyles.transformOrigin;
      this.el.style.transformStyle = this._originalInlineStyles.transformStyle;
      this.el.style.webkitBackdropFilter =
        this._originalInlineStyles.webkitBackdropFilter;
    }
  ...
  window.liquidGL = function (userOptions = {}) {
    ...
    if (!renderer) {
      renderer = new liquidGLRenderer(options.snapshot, options.resolution, {
        allowFixedSnapshot: options.allowFixedSnapshot,
      });
  ...
  window.liquidGL.refresh = function (config = {}) {
    const renderer = window.__liquidGLRenderer__;
    if (!renderer) return;
    renderer.lenses.forEach((lens) => lens.updateMetrics());
    if (config.capture && renderer.captureSnapshot) {
