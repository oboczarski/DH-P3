(function(){
  ...
  const calculatePlayerScore = (player) => clamp(Math.round((player.consistency + player.ppg * 3 + player.ceiling) / 3), 0, 99);
  ...
  const ppgBarData = (filter) => [...players]
  ...
  const setWidth = (id, pct) => { const el = document.getElementById(id); if (el) el.style.width = `${clamp(pct, 0, 100)}%`; };
  ...
  function drawScatterChart(containerId, data) {
    ...
    const margin = {
      ...
      right: width * 0.05,
      bottom: isMobile ? 40 : height * 0.1,
      left: isMobile ? 40 : width * 0.06
    };
    const innerWidth = width - margin.left - margin.right;
    const innerHeight = height - margin.top - margin.bottom;
    ...
    const x = d3.scaleLinear().domain([50, 100]).range([0, innerWidth]);
    const y = d3.scaleLinear().domain([20, 70]).range([innerHeight, 0]);
    ...
    g.append('text')
      .attr('x', innerWidth / 2)
      .attr('y', innerHeight + (isMobile ? 35 : margin.bottom - 5))
      .attr('text-anchor', 'middle')
      .attr('fill', '#94a3b8')
    ...
    g.append('text')
      ...
      .attr('y', isMobile ? -30 : -margin.left + 20)
      .attr('text-anchor', 'middle')
      .attr('fill', '#94a3b8')
      .attr('font-size', isMobile ? '8px' : '16px')
      .attr('font-weight', 'bold')
    ...
    g.selectAll('.scatter-dot')
      ...
      .attr('r', 0)
      .attr('fill', d => colorMap[d.position] || '#22d3ee')
      .transition()
      .duration(1000)
      .delay((d, i) => i * 30)
      .ease(d3.easeBackOut)
    ...
    const sim = d3.forceSimulation(labelNodes)
      .force('x', d3.forceX(d => x(d.consistency)).strength(3))
      .force('y', d3.forceY(d => y(d.ceiling) - 10).strength(3))
  ...
  // --- Init entrypoint ---
  function initHomeDashboard(data) {
    const root = document.getElementById('fc-dashboard-root');
    if (!root) return;
    players = data && Array.isArray(data) ? data : defaultPlayers;
